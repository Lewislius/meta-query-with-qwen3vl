...
# Qwen3-VL support: try importing the model class
try:
...
class MLLMInContext(PreTrainedModel):
    ...
    def __init__(
        ...
        self.config = config
        if "Qwen3-VL" in config.mllm_id:
            self.mllm_type = "qwen3vl"
        elif "Qwen2.5-VL" in config.mllm_id:
        ...
        if self.mllm_type == "llavaov":
            ...
            # Disable sliding window for full-context attention on MetaQueries
            ...
            # Qwen3-VL uses patch_size=16, factor=28 (same pixel budget as Qwen2.5-VL)
            ...
            # Qwen3-VL hidden sizes:
            # 2B: 1536, 4B: 2560, 8B: 4096
    ...
    def tokenize(
        ...
        if tokenizer.mllm_type in ("qwenvl", "qwen3vl") and "pixel_values" in text_inputs:
